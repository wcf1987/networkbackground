...
public interface NetworkInterMapper {
    ...
    //增加一个Person
    @Insert("insert into "+TbaleName+"(ID,Name,Type,IP,Port,Protocol,Describes,CreateTime,AuthorID)values(null,#{Name},#{Type},#{IP},#{Port},#{Protocol},#{Describes},DATE_FORMAT(now(),'%Y-%m-%d %H:%i:%S') ,#{AuthorID})")
    ...
    //更改一个Person
    @Update("update "+TbaleName+" set Name =#{Name},  IP =#{IP} ,  Port =#{Port},Protocol=#{Protocol}, Describes =#{Describes}  where ID=#{ID}")
